...
class DataHelper:
    ...
    def get_generalized_loader(self):
        set1 = self._get_clean_base_set()
        set2 = self._get_noisy_novel_set()
        print(f'train base: {len(set1.image_list)}, train novel: {len(set2.image_list)}')
        catted_set = cat_datasets(set1, set2)
        return Loader(catted_set, batch_size = self.batch_size, shuffle = True, num_workers = self.num_workers)
    ...
    def get_generalized_base_test_loader(self):
        data_set = self._get_base_test_set()
        generalized_set = DataSet(data_set.root_path, self.base_categories + self.novel_categories, data_set.transform)
        generalized_set.image_list = []
        for p, c in data_set.image_list:
            generalized_set.image_list.append((p, generalized_set.category2int[data_set.int2category[c]]))
        return Loader(generalized_set, batch_size = self.batch_size, shuffle = True, num_workers = self.num_workers)

    def get_generalized_novel_test_loader(self):
        data_set = self._get_novel_test_set()
        generalized_set = DataSet(data_set.root_path, self.base_categories + self.novel_categories, data_set.transform)
        generalized_set.image_list = []
        for p, c in data_set.image_list:
...
def split_dataloader(data_loader, val_rate = 0.2):
    ...
    return Loader(train_set, batch_size = data_loader.batch_size, shuffle = True,
