...
class AttributeHead(nn.Module):
    def __init__(self, args, diff_dim = 2048):
        ...
        if args.domain_bn == 1:
            ...
            self.bn = None
        self.relu = nn.ReLU(inplace = True)
        self.fc2 = nn.Linear(diff_dim, args.dim_att)
...
def make_binary_similarities(categories):
    pair = pair_enumeration(categories.unsqueeze(1))
    return (pair[:, 0] == pair[:, 1]).long()


def make_cosine_similarities(categories, attributes):
    pair = pair_enumeration(categories.unsqueeze(1))
    dot = torch.sum(attributes[pair[:, 0]] * attributes[pair[:, 1]], dim = 1)
    norms = torch.norm(attributes[pair[:, 0]], dim = 1) * torch.norm(attributes[pair[:, 1]], dim = 1)
    cos_sim = dot / norms
    # 需要preprocessing.normalize将att放缩到0到1
    # 这样cos_sim也属于0到1
    return torch.stack([1 - cos_sim, cos_sim], dim = 1)
